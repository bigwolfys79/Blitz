...
PREDICTION_ADJUSTMENT = {
    ...
    'MIN_MATCH_TO_ADJUST': 2, # Минимум совпадений в предыдущих предсказаниях для активации корректировки
    
    # === Стратегии замены чисел ===
    'ADJUSTMENT_STRATEGY': 'smart', # Стратегия выбора чисел:
                                    # 'smart' - умный выбор, 'hot' - только горячие,
                                    # 'cold' - только холодные, 'random' - случайные
    
    # === Критерии для проблемных чисел ===
    'MIN_SUCCESS_RATE': 25,  # Минимальный процент успешных предсказаний для числа (0-100)
    'MAX_MISSES': 10,         # Максимальное допустимое количество промахов подряд
    ...
    'MIN_ATTEMPTS': 30       # Минимальное количество попыток предсказания числа 
